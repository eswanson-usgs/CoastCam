"""
Eric Swanson
Purpose: Convert the filename for an image at Madeira beach to the format used by other CoastCam cameras

Description:
The purpose of this script is to convert the S3 filepath for all images in a folder in the
USGS Madeira Beach CoastCam bucket.
...
[unix datetime].[Day of week].[Month].[DayOfMonth]_[Hour]_[Minute]_[second].[Year].I2Rgus.[Camera#long].[image type].[file extension]
The new filenames are in the format [unix datetime].[camera#].[image type].[file extension]
...
def copy_MadBeach_image(source_filepath):
    """
...
for image in image_list:
    ...
    if image.endswith('.txt') or isImage == False:
        ...
        dest_filepath = copy_MadBeach_image(source_filepath)
